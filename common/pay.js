...
export const pay = (result, self, success, fail) => {
	...
	if (result.code == 1) {
		// #ifdef MP-ALIPAY
			aliAppPayXCX(result, self,success, fail);
		// #endif
	}
}
...
// 支付宝小程序支付
function aliAppPayXCX(result, self,success, fail){
	// console.log(result,"kankan")
	uni.requestPayment({
		provider: 'alipay',
		orderInfo: result.data.alipay_trade_create_response.trade_no,
		success(res) {
