	...
	<view class="u_live">
		...
		<view class="uLive_top" v-if="scrollTopNum>=185">
			<scroll-view scroll-x="true" class="">
				<view class="d-s-c">
					...
					<view :class="category_id==item.category_id?'type_item_active':'type_item'" v-for="(item,index) in categorytype"
					 :key="index" @click="chooseType(item.category_id)">{{item.name}}</view>
				</view>
			</scroll-view>
		</view>
		<view class="live_content" v-if="categorytype!=''">
			...
			<view class="live_list " v-if="!isLoading&&typelist">
				<view @click="toRoom(liveitem.room_id)" class="live_item" v-for="(liveitem,liveindex) in typelist" :key="liveindex"
				 v-if="liveitem.live_status!=107">
					...
					<view class="live_item_img">
						<image :src="liveitem.share.file_path" mode="aspectFill"></image>
					</view>
					<view class="d-b-c live_item_b">
						<view class="live_item_bl">
							...
							<view class="d-s-c live_item_blb">
								<view class="live_item_blbimg">
									<image :src="liveitem.user.avatarUrl" mode=""></image>
								</view>
						...
						<view class="live_item_brbox" v-if="liveitem.product[0]">
							<view class="live_item_br">
								<image :src="liveitem.product[0].product.image[0].file_path" mode=""></image>
							</view>
	...
	export default {
		...
		methods: {
			...
			getData() {
				...
				self._post('plus.live.Room/lists', {
					...params
				}, function(res) {
			...
			toRoom(id) {
				uni.navigateTo({
					url: '/pagesLive/live/live?room_id=' + id + "&sence=join"
				})
			}
		}
	}
