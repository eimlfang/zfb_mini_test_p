	...
	<view class="diy-banner-box pr" :class="itemData.style.imgShape"
		...
		<swiper class="swiper" :autoplay="autoplay" :interval="interval" :duration="duration" @change="changeSwiper"  :style="itemData.style.imgShape=='square'?'height:'+itemData.style.height+'rpx;':'height:'+(itemData.style.height*0.92)+'rpx;'">
			<swiper-item v-for="(item,index) in itemData.data" :key="index" >
				<image @click="gotoPages(item)" :style="itemData.style.imgShape=='square'?'height:'+itemData.style.height+'rpx;':'height:'+(itemData.style.height*0.92)+'rpx;'" :src="item.imgUrl"></image>
			</swiper-item>
		</swiper>
...
<script>
	export default {
		...
		created() {
			...
			var newcolor = this.rgbcolor(this.itemData.data[0].background);
			this.background = 'linear-gradient(' + this.itemData.data[0].background +','+ newcolor + ')';
			// this.background = this.itemData.data[0].background;
		},
		methods: {
			changeSwiper(e) {
				...
				var newcolor = this.rgbcolor(background);
				this.background = 'linear-gradient(' + background +','+ newcolor + ')';
				// this.background = background;
				uni.$emit('updatebg', background);
			},
			rgbcolor(bgColor){
			...
			/*跳转页面*/
			gotoPages(e) {
				console.log(e,"console.log(e.linkUrl);");
