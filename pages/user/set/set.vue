<template>
	<view class="address-form" :data-theme='theme()' :class="theme() || ''">
			...
			<view class="d-b-c border-b info-item avatar">
				...
				<view class="d-e-c" @click="changeAvatarUrl">
					<view class="info-image">
						<image style="width: 100rpx;height: 100rpx;border-radius: 50%;" :src="baseurl + userInfo.avatarUrl"></image>
		...
		<!-- 	<view class="d-b-c p-30-0 border-b">
				...
				<view class="d-e-c" v-if="userInfo.address[0].phone">
					<text class="mr20">{{userInfo.address[0].phone}}</text>
				</view>
				<view class="d-e-c" v-if="!userInfo.address[0].phone" @click="gotoBind">
					<text class="mr20">未绑定</text>
					<text class="iconfont icon-jiantou"></text>
				</view>
			</view> -->
			<view class="d-b-c p-30-0" @click="gotoBindUser">
				<text class="key-name" >实名认证</text>
				<view class="d-e-c" v-if="userInfo.username">
					<text class="mr20">{{userInfo.username}}</text>
				</view>
				<view class="d-e-c" @click="gotoBindUser" v-if="!userInfo.username">
					<text class="mr20">未绑定</text>
					<text class="iconfont icon-jiantou"></text>
				</view>
			</view>
		...
		<Popup :show="isPopup" type='bottom' :width="750" :padding="0" @hidePopup="hidePopupFunc">
			<view class="d-s-s d-c p20 mpservice-wrap">
				...
				<view class="d-a-c ww100">
					<button class="btn-gray-border" style="width: 100rpx;height: 70rpx;line-height: 70rpx;"
						@click="hidePopupFunc">取消</button>
					<button class="theme-btn" style="width: 100rpx;height: 70rpx;line-height: 70rpx;"
						@click="subName">确认</button>
				</view>
			</view>
		</Popup>
		<!-- 上传头像 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			onGetPhoneAuth() {
				my.getPhoneNumber({
					success: (res) => {
						...
						console.log(JSON.parse(encryptedData));
			...
			gotoBindUser() {
				this.gotoPage("/pages/user/modify-phone/userAuther");
			},
			logout() {
