		...
		<view class="form-wrap f30">
			<form @submit="formSubmit" @reset="formReset">
				<view class="p-0-20 pt30 txje">
					{{ words.cash_apply.words.money.value }}
				</view>
				<view class="p-0-20 ">
					<view class="withd-bc">
						...
						<!-- <template v-if="hasType('20')">
							<view class="p-30-0 border-b">
								...
								<template v-if="withdraw_type==20">
									<view class="mt20">
										<input class="p20 border" name="alipay_name" type="text" value="" placeholder-class="grary" placeholder="请输入姓名" />
									</view>
									<view class="mt20">
										<input class="p20 border" name="alipay_account" type="text" value="" placeholder-class="grary" placeholder="请输入支付宝账号" />
									</view>
								</template>
							</view>
						</template> -->
						<template v-if="hasType('30')">
							<view class="p-30-0 border-b">
								...
								<template v-if="withdraw_type==30">
									<!-- <view class="mt20">
										<input class="p20 border" name="bank_name" type="text" value="" placeholder-class="grary" placeholder="请输入姓名" />
									</view> -->
									<view class="mt20">
										<input class="p20 border" name="bank_account" type="text" value="" placeholder-class="grary" placeholder="请输入开户行名称/地址" />
									</view>
									<view class="mt20">
										<input class="p20 border" name="bank_card" type="text" value="" placeholder-class="grary" placeholder="请输入银行卡号" />
									</view>
									<view class="mt20">
										<input class="p20 border" name="name" type="text" value="" placeholder-class="grary" placeholder="请输入姓名" />
									</view>
								</template>
							</view>
						</template>
	...
	export default {
		...
		methods: {
			...
			formSubmit(e) {
				...
				self._post('/user.user/withdrawalApplication',{
					price:data.money,
					cardNumber:data.bank_card,
					bankOfDeposit:data.bank_account,
					name:data.name
				},function(data){
					uni.hideLoading();
					uni.showToast({
