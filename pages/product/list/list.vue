		...
		<view class="top-box">
			<view class="index-search-box index-search-box_re d-b-c" id="searchBox">
				<view class="index-search index-search_re t-c flex-1">
					...
					<input type="text" style="background-color: #F2F2F2;" v-model="search" class="flex-1 ml10 f26 gray3" value="" placeholder-class="f26 gray9"
					 placeholder="搜索商品" confirm-type="search" @confirm="search()" />
				</view>
			</view>
			<view class="inner-tab">
				...
				<view class="item">
					<view class="box" style="height: 100%;">
						<image style="width: 36rpx;height: 36rpx;" @click="select_type()" :src="isLieBiao == true?'/static/shop/liebiao.png':'/static/shop/tubiao.png'" mode="widthFix"></image>
					</view>
				</view>
			</view>
		</view>
			 ...
			 @scrolltolower="scrolltolowerFunc">
				...
				<view class="shop_body" v-if="isLieBiao ==true">
					<view class="shop_body_l_item" :class="index==listData.length-1?'noborder':''" v-for="(item,index) in listData"
					 :key="index" @click="gotoList(item.product_id)" v-if="item.hide != 1">
						<view>
							<image :src="item.product_image" mode="widthFix"></image>
						</view>
						<view class="shop_body_l_item_info">
							...
							<view class="d-b-c pb10">
								<view class="shop_body_l_item_info_price">
									<view class="f24 shop_red">¥<text class="f32 fb">{{item.product_price}}/天起</text></view>
								</view>
				...
				<view class="shop_body2" v-if="isLieBiao ==false">
					<view class="shop_body_t_item" :class="index % 2 == 0?'ml20 mr20':' mr20'" v-for="(item,index) in listData" :key="index"
					 @click="gotoList(item.product_id)" v-if="item.hide != 1">
						<image :src="item.product_image" mode="widthFix"></image>
						<view class="shop_body_t_item_info">
							...
							<view class="shop_body_t_item_info_price">
								<view class="f20 redF6">¥<text class="f32">{{item.product_price}}/天起</text></view>
								<view class="f20 huaxianjia">¥<text class="24">{{item.line_price}}</text>/天起</view>
							</view>
						</view>
					</view>
				</view>
