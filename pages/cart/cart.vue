		...
		<view class="card" v-if="!loadding">
			<view :class="{ pb100: tableData.length > 0 }" style="position: relative;">
						...
						<view v-for="(supplier_item, supplier_index) in tableData" :key="supplier_index">
							<view class="supplier_list">
								<view class="supplier_list_tit">
									<text class="d-c-c" @tap="checkStprItem(supplier_item, supplier_index)">
										<checkbox color="red" value="cb" class="checkbox" :checked="supplier_item.checked" />
									</text>
									<view class="d-a-c" @tap="gotoPage('/pages/shop/shop?shop_supplier_id='+supplier_item.supplier.shop_supplier_id)">
								...
								<view class="item" v-for="(item, index) in supplier_item.productList" :key="index">
									<text class="d-c-c" @click="checkItem(item, supplier_index, index)">
									
										<checkbox color="red" value="cb" class="checkbox" :checked="item.checked" />
									</text>
									<view style="overflow: hidden;">
										<image @click="gotoPage('/pages/product/detail/detail?product_id='+item.product_id)" :src="item.product_image"
										 class="cover" mode="aspectFit"></image>
									</view>
									<view class="info">
										...
										<view class="level-box count_choose">
											...
											<view class="num-wrap">
												<view class="" @tap.stop="reduceFunc(item)" v-if="item.total_num > 1">
													<image class="reduce_icon" src="../../static/icon/reduce.png" mode="widthFix"></image>
												</view>
												<view class="text-wrap">{{item.total_num}}</view>
												<view class="" @tap.stop="addFunc(item)" v-if="item.total_num < item.product_sku.stock_num ">
													<image class="add_icon" src="../../static/icon/add.png" mode="widthFix"></image>
												</view>
											</view>
										</view>
									</view>
				...
				<view class="bottom-btns f28" v-if="tableData.length > 0">
					<text class="d-c-c mr20" @tap="onCheckedAll()">
					...
					<view class="pr20" v-else><button type="primary" @tap="onDelete()" class="delete-btn theme-btn mr20">删除</button></view>
				</view>
	...
	export default {
		...
		methods: {
			...
			checkItem(e, supplier_index, index) {
				// console.log(e,"看看哪款11111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111")
				e.checked = !e.checked;
