			...
			<view class="evaluate-item p30" v-for="(item, index) in tableData" :key="index">
				...
				<view class="grade evalu p-30-0 mt30">
					<view class="evalu-value">
						描述相符：
						<view class="eval">
							<i v-for="(itemEv,indexEv) in describe[index]" :key="indexEv" :class="itemEv?'icon iconfont icon-start':'icon iconfont icon-start1'"
							 @click="choosees(indexEv,index)"></i>
						</view>
					</view>
					<view class="evalu-value">
						服务态度：
						<view class="eval">
							<i v-for="(itemEv,indexEv) in service[index]" :key="indexEv" :class="itemEv?'icon iconfont icon-start':'icon iconfont icon-start1'"
					...
					<view class="evalu-value">
						配送服务：
						<view class="eval">
							<i v-for='(itemEv,indexEv) in logistics[index]' :key="indexEv" :class="itemEv?'icon iconfont icon-start':'icon iconfont icon-start1'"
							 @click="chooseLog(indexEv,index)"></i>
						</view>
					</view>
				</view>
